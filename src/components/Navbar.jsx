...
function Navbar({ restaurant }) {
    ...
    return (
        <nav className="bg-neutral-surface/90 backdrop-blur-md p-4 border-b border-gray-100 sticky top-0 z-50">
            <div className="max-w-7xl mx-auto grid grid-cols-3 items-center px-4 md:px-8">

                {/* Left Side: Table Number (Now subtle on the far left) */}
                ...
                <div className="flex items-center justify-center gap-8">
                    <Link to={`/${restaurantName}`} className="flex items-center gap-3 group">
                        <span className="text-xl font-headline font-extrabold text-secondary tracking-tighter">
                            {/* If restaurant data exists, use its real name, otherwise use the URL param */}
                            {restaurant?.name || restaurantName}
                        </span>
                    </Link>

                    <div className="hidden md:flex gap-6">
                        {['Menu', 'Track Order'].map((item) => (
                            <button key={item} className="text-xs font-bold text-gray-400 hover:text-primary transition-colors uppercase tracking-widest">
                                {item}
                            </button>
                        ))}
                    </div>
                ...
                <div className="flex items-center justify-end gap-6">
                    <div className="flex items-center gap-4 border-l border-gray-100 pl-6">
                        ...
                        {/* Profile Avatar: Replaced 'BU' with the Restaurant Logo */}
                        <button className="w-9 h-9 rounded-full bg-secondary overflow-hidden border border-gray-100 shadow-sm transition-transform hover:scale-105 flex items-center justify-center">
                            ...
                            <span className={`${logoSrc ? 'hidden' : 'flex'} text-[10px] font-bold text-white uppercase`}>
                                {restaurant?.name?.substring(0, 2) || 'BE'}
                            </span>
                        </button>
                    </div>
                </div>
